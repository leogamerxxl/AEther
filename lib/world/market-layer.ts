...
export function createMarketOverlay(phase: DayPhase): { overlay: MapboxOverlay; setVisible: (v: boolean) => void } {
  const overlay = new MapboxOverlay({ interleaved: true, layers: [buildLayer(true, phase)] });
  let cur = true;
  return {
    overlay,
    setVisible: (v: boolean) => {
      if (v === cur) return;
      cur = v;
      overlay.setProps({ layers: [buildLayer(v, phase)] });
    },
  };
}